...
#inicializa a conexão entre o seu aplicativo Python e o projeto Firebase.
cred = credentials.Certificate('Biblioteca/ChavePv.json')
...
class BookApp(MDApp):
    ...
    def add_book(self, instance):
        ...
        if title and author:
            ...
            new_book_ref = book_ref.push()#cria um novo nó com uma chave única gerada automaticamente no banco de dados.
        ...
        if books:
            for key, book_data in books.items():
                ...
                if self.search_input.text.strip().lower() in book.title.lower() or self.search_input.text.strip().lower() in book.author.lower():
                    ...
                    read_button.bind(on_release=lambda x, book=book: self.toggle_read(book))

                    delete_button = MDRaisedButton(text='Excluir', font_size=14, size_hint=(1, None), height=40)
                    delete_button.bind(on_release=lambda x, book=book: self.delete_book(book))
